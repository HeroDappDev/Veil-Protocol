...
import { encryptWithAES, decryptWithAES, deriveKeyForOwner, sha256Hash } from "./crypto-utils";
...
export async function executeTerminalCommand(
  ...
  switch (baseCommand) {
    ...
    case '/mykey':
      return await showEncryptionKey(encryptionOwnerId, userCode.code);
    
    case '/usekey':
      return await useEncryptionKey(encryptionOwnerId, args.join(' '), userCode.code);
    
    case '/privacytx':
      return await performPrivacyTransaction(walletAddress, encryptionOwnerId, args, userCode.code);
...
async function showEncryptionKey(encryptionOwnerId: string, code: string): Promise<TerminalResponse> {
  const encryptionKey = await storage.getEncryptionKeyByWallet(encryptionOwnerId);
...
RSA-2048 encryption key pair.

Once generated, your keys will be stored securely and tied to
your protected browser privacy session.`
    };
  }
...
This key is cryptographically bound to your protected browser session.
...
Or use: /mykey to check your key status`
  ...
  try {
    ...
    await storage.updateKeyLastUsed(encryptionOwnerId);
    
    // Generate unique encrypted output using the protected session-derived key
    const sessionKey = deriveKeyForOwner(encryptionOwnerId);
    const encrypted = encryptWithAES(data, sessionKey);
...
specific to your protected browser session.`,
      metadata: { 
        encrypted, 
        txId, 
        algorithm: encryptionKey.keyType 
